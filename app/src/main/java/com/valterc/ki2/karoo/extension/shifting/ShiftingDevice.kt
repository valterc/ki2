...
class ShiftingDevice(
    ...
    fun connect(emitter: Emitter<DeviceEvent>) {
        ...
        val devicePreferencesListener = BiConsumer<DeviceId, DevicePreferencesView> { deviceId: DeviceId, preferences: DevicePreferencesView ->
        ...
        val connectionInfoListener = BiConsumer<DeviceId, ConnectionInfo> { deviceId: DeviceId, connectionInfo: ConnectionInfo ->
            if (this@ShiftingDevice.deviceId != deviceId){
                return@BiConsumer
            }

            Timber.i("[%s] Emitting connection %s", deviceId.uid, connectionInfo.connectionStatus)
            ...
            when(connectionInfo.connectionStatus) {
                com.valterc.ki2.data.connection.ConnectionStatus.INVALID -> emitter.onNext(OnConnectionStatus(ConnectionStatus.DISCONNECTED))
                com.valterc.ki2.data.connection.ConnectionStatus.NEW -> emitter.onNext(OnConnectionStatus(ConnectionStatus.SEARCHING))
                com.valterc.ki2.data.connection.ConnectionStatus.CONNECTING -> emitter.onNext(OnConnectionStatus(ConnectionStatus.SEARCHING))
                com.valterc.ki2.data.connection.ConnectionStatus.ESTABLISHED -> emitter.onNext(OnConnectionStatus(ConnectionStatus.CONNECTED))
                com.valterc.ki2.data.connection.ConnectionStatus.CLOSED -> emitter.onNext(OnConnectionStatus(ConnectionStatus.DISCONNECTED))
                else -> emitter.onNext(OnConnectionStatus(ConnectionStatus.DISCONNECTED))
            }
        }

        val batteryInfoListener = BiConsumer<DeviceId, BatteryInfo> { deviceId: DeviceId, batteryInfo: BatteryInfo ->
            if (this@ShiftingDevice.deviceId != deviceId){
                return@BiConsumer
            }
            ...
            val criticalLevel = preferencesView?.getBatteryLevelCritical(extensionContext.context)
        ...
        val shiftingInfoListener = BiConsumer<DeviceId, ShiftingInfo> { deviceId: DeviceId, shiftingInfo: ShiftingInfo ->
            if (this@ShiftingDevice.deviceId != deviceId){
                return@BiConsumer
            }
            ...
            if (shiftingGearingHelper.hasFrontGearSize()) {
                frontGearFieldMap[DataType.Field.SHIFTING_FRONT_GEAR_TEETH] = shiftingGearingHelper.frontGearTeethCount.toDouble()
            }
            ...
            if (shiftingGearingHelper.hasRearGearSize()) {
                rearGearFieldMap[DataType.Field.SHIFTING_REAR_GEAR_TEETH] = shiftingGearingHelper.rearGearTeethCount.toDouble()
            }
        ...
        val job = CoroutineScope(Dispatchers.IO).launch {
            ...
            extensionContext.serviceClient.registerDevicePreferencesWeakListener(devicePreferencesListener)
        ...
        emitter.setCancellable {
            ...
            job.cancel()
            extensionContext.serviceClient.unregisterDevicePreferencesWeakListener(devicePreferencesListener)
