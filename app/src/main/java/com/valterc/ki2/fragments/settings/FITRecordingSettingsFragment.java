...
public class FITRecordingSettingsFragment extends PreferenceFragmentCompat {
    @Override
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        setPreferencesFromResource(R.xml.preferences_fit_recording, rootKey);
        Preference preferenceFITRecording = findPreference(getString(R.string.preference_fit_recording));
        Objects.requireNonNull(preferenceFITRecording).setOnPreferenceChangeListener((preference, newValue) -> {
            if (Objects.equals(newValue, Boolean.TRUE)) {
                new AlertDialog.Builder(requireContext(), R.style.AlertDialogStyle)
                        .setTitle(R.string.text_fit_recording)
                        .setMessage(getString(R.string.text_warning_fit_recording))
